...
function ensureRequiredConfiguration(options = {}) {
  ...
  if (requiresSupabase && !supabaseAdmin) {
    issues.push('Defina SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY para processar webhook/benef├¡cios e (se habilitado) validar JWT no proxy.');
  }

  if (isProduction && allowedOrigins.size === 0) {
    issues.push('Defina BABYLON_ALLOWED_ORIGINS com os dom├¡nios autorizados do frontend.');
  }
...
const server = createServer(async (req, res) => {
  ...
  if (requiredConfigurationIssues.length) {
    ...
    res.writeHead(500);
    res.end(JSON.stringify({
      error: 'Configura├º├úo incompleta',
  ...
  if (req.url === '/webhooks/babylon' && req.method === 'POST') {
    ...
    try {
      ...
      res.writeHead(400);
      res.end(JSON.stringify({
        error: 'Webhook inv├ílido',
        message: error?.message || 'N├úo foi poss├¡vel processar payload JSON',
      }));
    }
    return;
  }
  ...
  if (req.url === '/api/public/first-offer/checkout' && req.method === 'POST') {
    try {
      ...
      if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.writeHead(400);
        res.end(JSON.stringify({ error: 'E-mail inv├ílido' }));
        return;
      }
      ...
      if (!upstreamResponse.ok) {
        ...
        res.end(JSON.stringify({
          error: 'Falha ao criar transa├º├úo na Babylon',
          detail: upstreamData,
        }));
        return;
      }
      ...
      res.writeHead(500);
      res.end(JSON.stringify({
        error: 'Falha no checkout p├║blico',
        message: error?.message || 'Erro desconhecido',
      }));
    }
    return;
  ...
  try {
    ...
    if (req.method === 'POST' && isTransactionCreatePath(upstreamPath)) {
      ...
      if (!checkoutOrderId) {
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.writeHead(400);
        res.end(JSON.stringify({ error: 'external_id inv├ílido para transa├º├úo Babylon' }));
        return;
      }
      ...
      if (authenticatedUser?.id) {
        ...
        if (checkoutOrderError || !checkoutOrder?.id) {
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
          res.writeHead(403);
          res.end(JSON.stringify({ error: 'Forbidden: checkout order n├úo pertence ao usu├írio autenticado' }));
          return;
        }
      }
    }
    ...
    if (req.method === 'POST' && upstreamPath === '/claim-pending-benefits') {
      ...
      if (error) {
        ...
        res.writeHead(500);
        res.end(JSON.stringify({
          error: 'Erro ao aplicar benef├¡cios pendentes',
          message: error.message,
        }));
        return;
      }
