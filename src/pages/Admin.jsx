...
export function Admin() {
    ...
    const [monthlyRevenueCents, setMonthlyRevenueCents] = useState(0);
    ...
    useEffect(() => {
        ...
        if (activeTab === 'alerts') fetchSecurityAlerts();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [activeTab]);
    ...
    function createStoreEditFromProduct(product) {
        return {
            ...
            allow_monthly: product?.metadata?.allow_monthly !== false,
        };
    }
    ...
    async function fetchMonthlyRevenue() {
        ...
        const { data, error } = await supabase
            ...
            .select('id, provider_name, metadata, paid_at, created_at')
            .eq('status', 'paid')
        ...
        const totalCents = (data || []).reduce((sum, row) => {
            ...
            const topupAmountCents = toSafeCents(metadata?.topup_amount_cents);
            const payableAmountCents = toSafeCents(metadata?.total_payable_cents);
            const planAmountCents = toSafeCents(metadata?.total_plan_cents);

            if (source === 'wallet_topup') {
                return sum + topupAmountCents;
            }

            const checkoutAmount = payableAmountCents || planAmountCents;
            return sum + checkoutAmount;
        }, 0);

        setMonthlyRevenueCents(totalCents);
        setRevenueLoading(false);
    }
    ...
    async function saveStoreProduct(product) {
        ...
        metadataBase.allow_monthly = edit.allow_monthly !== false;
    ...
    async function createStoreProduct() {
        ...
        const metadata = {
            ...
            allow_monthly: storeCreateForm.allow_monthly !== false,
        };
    ...
    function buildAlert(id, severity, title, description, details = []) {
        return { id, severity, title, description, details };
    }
    ...
    function formatProfileIdentity(profileId, profileById) {
        const profile = profileById.get(profileId);
        if (!profile) return `Usuário ${profileId}`;
        ...
        if (fullName) return `${fullName} (${profileId})`;
        return `Usuário ${profileId}`;
    }
    ...
    async function fetchSecurityAlerts() {
        ...
        const now = Date.now();
        const stalePendingCutoffIso = new Date(now - (30 * 60 * 1000)).toISOString();
        ...
        const [
            ...
            webhookWithoutOrderResult,
        ] = await Promise.all([
            supabase
                .from('checkout_orders')
                .select('id, profile_id, provider_name, created_at')
                .eq('status', 'pending')
                .lt('created_at', stalePendingCutoffIso)
                .order('created_at', { ascending: true })
                .limit(30),
            supabase
                .from('checkout_orders')
                .select('id, profile_id, provider_name, created_at, metadata')
                .eq('status', 'failed')
            ...
            supabase
                ...
                .select('id, profile_id, amount, source, created_at, metadata')
            ...
            supabase
                ...
                .gte('created_at', last24hIso)
                .ilike('note', 'Acesso automático via%')
                .order('created_at', { ascending: false })
                .limit(80),
            supabase
                .from('checkout_webhook_events')
                .select('id, provider_name, event_type, received_at, provider_order_id')
                .gte('received_at', last24hIso)
        ...
        const queryErrors = [
            ...
            manualCreditsResult.error,
            autoAssignmentsResult.error,
            webhookWithoutOrderResult.error,
        ].filter(Boolean);
        ...
        if (queryErrors.length > 0) {
            alerts.push(buildAlert(
                'alerts-query-error',
                'high',
                'Falha ao carregar parte dos alertas',
                'Uma ou mais consultas de monitoramento falharam. Revise permissões/RLS e conexão do Supabase.',
                queryErrors.map((err) => String(err?.message || 'erro desconhecido'))
            ));
        }

        const pendingOrders = pendingOrdersResult.data || [];
        const failedOrders = failedOrdersResult.data || [];
        const manualCredits = manualCreditsResult.data || [];
        const autoAssignments = autoAssignmentsResult.data || [];

        const profileIds = Array.from(new Set([
            ...pendingOrders.map((row) => row.profile_id).filter(Boolean),
            ...failedOrders.map((row) => row.profile_id).filter(Boolean),
            ...manualCredits.map((row) => row.profile_id).filter(Boolean),
            ...autoAssignments.map((row) => row.profile_id).filter(Boolean),
        ]));
        ...
        if (profileIds.length > 0) {
            ...
            if (profilesResult.error) {
                alerts.push(buildAlert(
                    'alerts-profiles-lookup-error',
                    'medium',
                    'Falha ao identificar usuários nos alertas',
                    'Os alertas foram carregados, mas não foi possível trazer email/nome para todos os usuários.',
                    [String(profilesResult.error?.message || 'erro desconhecido')]
                ));
            } else {
        ...
        if (pendingOrders.length > 0) {
            alerts.push(buildAlert(
                ...
                'high',
                'Pedidos pendentes há mais de 30 min',
                `${pendingOrders.length} pedido(s) ficaram pendentes além da janela esperada. Pode indicar webhook falhando ou fluxo travado.`,
                pendingOrders.slice(0, 6).map((row) => `Pedido ${row.id} • ${formatProfileIdentity(row.profile_id, profileById)} • ${formatAlertDateTime(row.created_at)}`)
            ));
        }
        ...
        if (failedOrders.length >= 5) {
            alerts.push(buildAlert(
                'failed-orders-spike',
                'medium',
                'Pico de pedidos com falha (24h)',
                `${failedOrders.length} pedido(s) com status failed nas últimas 24h.`,
                failedOrders.slice(0, 6).map((row) => `Pedido ${row.id} • ${formatProfileIdentity(row.profile_id, profileById)} • ${row.provider_name || 'provedor'} • ${formatAlertDateTime(row.created_at)}`)
            ));
        }
        ...
        if (manualCredits.length > 0) {
            alerts.push(buildAlert(
                'manual-credits',
                'medium',
                'Créditos manuais adicionados (7 dias)',
                `${manualCredits.length} lançamento(s) de crédito manual detectado(s). Validar se todos foram autorizados.`,
                manualCredits.slice(0, 6).map((row) => `${formatProfileIdentity(row.profile_id, profileById)} • +${row.amount} créditos • ${formatAlertDateTime(row.created_at)}`)
            ));
        }

        const assignmentOrderPairs = autoAssignments
            .map((row) => ({
                assignmentId: row.id,
                profileId: row.profile_id,
                createdAt: row.created_at,
                orderId: extractUuidFromText(row.note),
            }))
            .filter((row) => !!row.orderId);

        const uniqueOrderIds = Array.from(new Set(assignmentOrderPairs.map((row) => row.orderId)));
        let orderStatusById = new Map();

        if (uniqueOrderIds.length > 0) {
            const linkedOrdersResult = await supabase
                .from('checkout_orders')
                .select('id, profile_id, status')
                .in('id', uniqueOrderIds);

            if (linkedOrdersResult.error) {
                alerts.push(buildAlert(
                    'assignment-order-lookup-error',
                    'high',
                    'Falha ao validar concessões automáticas',
                    'Não foi possível conferir se acessos concedidos possuem pedido pago correspondente.',
                    [String(linkedOrdersResult.error?.message || 'erro desconhecido')]
                ));
            } else {
                orderStatusById = new Map((linkedOrdersResult.data || []).map((row) => [row.id, row]));
            }
        }

        const inconsistentAssignments = assignmentOrderPairs.filter((row) => {
            const linkedOrder = orderStatusById.get(row.orderId);
            if (!linkedOrder) return true;
            if (linkedOrder.status !== 'paid') return true;
            if (linkedOrder.profile_id !== row.profileId) return true;
            return false;
        });

        if (inconsistentAssignments.length > 0) {
            alerts.push(buildAlert(
                'inconsistent-auto-assignments',
                'critical',
                'Acesso automático sem pedido pago válido',
                `${inconsistentAssignments.length} concessão(ões) automática(s) sem correspondência íntegra com pedido pago.`,
                inconsistentAssignments.slice(0, 6).map((row) => `Assignment ${row.assignmentId} • ${formatProfileIdentity(row.profileId, profileById)} • pedido ${row.orderId} • ${formatAlertDateTime(row.createdAt)}`)
            ));
        }

        const webhookWithoutOrder = webhookWithoutOrderResult.data || [];
        if (webhookWithoutOrder.length > 0) {
            alerts.push(buildAlert(
                'webhook-without-provider-order',
                'low',
                'Webhooks sem provider_order_id (24h)',
                `${webhookWithoutOrder.length} evento(s) de webhook chegaram sem provider_order_id. Revise integração do gateway.`,
                webhookWithoutOrder.slice(0, 6).map((row) => `${row.provider_name || 'provider'} • ${row.event_type || 'evento'} • ${formatAlertDateTime(row.received_at)}`)
            ));
        }
    ...
    async function grantWalletCreditToUser() {
        if (!selectedUser?.id) return;

        const amount = Number.parseInt(walletCreditInput, 10);
        if (!Number.isFinite(amount) || amount <= 0) {
            alert('Informe um valor de crédito maior que zero.');
            return;
        }

        setWalletCreditSaving(true);
        const { data, error } = await supabase.rpc('admin_grant_wallet_credit', {
            p_profile_id: selectedUser.id,
            p_amount: amount,
            p_description: 'Crédito manual concedido no painel Admin',
        });

        if (error) {
            const errorMessage = String(error.message || '').toLowerCase();
            if (errorMessage.includes('admin_grant_wallet_credit')) {
                alert('Função de crédito não encontrada no banco. Execute setup_wallet_credit_checkout.sql.');
            } else {
        ...
        if (Number.isFinite(nextBalance)) {
            setUserWalletBalance(nextBalance);
        } else {
    ...
    const walletCreditPreview = Math.max(0, Number.parseInt(walletCreditInput || '0', 10) || 0);

    const alertSeverityOrder = useMemo(() => ({
        critical: 0,
        high: 1,
        medium: 2,
        low: 3,
    ...
    const alertSeverityMeta = {
        critical: {
            label: 'Crítico',
            countTone: 'text-red-300',
            cardTone: 'border-red-500/30 bg-red-500/5',
            sectionTitleTone: 'text-red-300',
        },
        high: {
            label: 'Alto',
            countTone: 'text-orange-300',
            cardTone: 'border-orange-500/30 bg-orange-500/5',
            sectionTitleTone: 'text-orange-300',
        },
        medium: {
            label: 'Médio',
            countTone: 'text-yellow-300',
            cardTone: 'border-yellow-500/30 bg-yellow-500/5',
            sectionTitleTone: 'text-yellow-300',
        },
        low: {
            label: 'Baixo',
            countTone: 'text-blue-300',
            cardTone: 'border-blue-500/30 bg-blue-500/5',
            sectionTitleTone: 'text-blue-300',
        },
    };

    const orderedSecurityAlerts = useMemo(() => {
        return [...securityAlerts].sort((a, b) => {
            const rankA = alertSeverityOrder[a.severity] ?? 99;
            const rankB = alertSeverityOrder[b.severity] ?? 99;
            if (rankA !== rankB) return rankA - rankB;
            return String(a.title || '').localeCompare(String(b.title || ''));
        });
    }, [securityAlerts, alertSeverityOrder]);

    const alertsBySeverity = useMemo(() => {
        return {
            critical: orderedSecurityAlerts.filter((item) => item.severity === 'critical'),
            high: orderedSecurityAlerts.filter((item) => item.severity === 'high'),
            medium: orderedSecurityAlerts.filter((item) => item.severity === 'medium'),
            low: orderedSecurityAlerts.filter((item) => item.severity === 'low'),
        };
    }, [orderedSecurityAlerts]);

    const topPriorityAlert = orderedSecurityAlerts[0] || null;
        ...
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2 bg-white/5 p-1 rounded-xl border border-white/10">
                    ...
                    <button
                        ...
                        className={cn(
                            "px-4 py-2 rounded-lg text-sm font-medium transition-colors",
                            activeTab === 'alerts' ? "bg-primary text-white shadow-lg" : "text-gray-400 hover:text-white"
                        )}
                    >
                        Alertas
                    </button>
                ...
                <Card className="bg-gradient-to-br from-green-500/20 to-transparent border-green-500/20">
                    <CardContent className="p-6 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-3xl font-bold text-white">{revenueLoading ? '...' : formatBRLFromCents(monthlyRevenueCents)}</p>
                        </div>
            ...
            {activeTab === 'alerts' && (
                <Card>
                    <CardHeader className="flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <CardTitle className="flex items-center gap-2">
                                <AlertTriangle className="h-5 w-5 text-primary" /> Alertas de Segurança e Pagamento
                            </CardTitle>
                            <p className="text-xs text-gray-400 mt-2">Resumo visual com prioridade de ação: Crítico → Alto → Médio → Baixo.</p>
                        </div>
                        <div className="flex items-center gap-2">
                            <p className="text-xs text-gray-400">Última atualização: {formatAlertDateTime(alertsLastUpdatedAt)}</p>
                            <Button type="button" variant="ghost" onClick={fetchSecurityAlerts} disabled={alertsLoading}>
                    ...
                    <CardContent>
                        {alertsLoading ? (
                            ...
                            <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-4 text-sm text-green-200 flex items-start gap-2">
                                <CheckCircle className="h-4 w-4 mt-0.5" />
                                <span>Nenhum alerta crítico no momento. Recomendado manter revisão diária desta aba.</span>
                            </div>
                        ) : (
                            <div className="space-y-5">
                                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                                    {(['critical', 'high', 'medium', 'low']).map((level) => {
                                        const meta = alertSeverityMeta[level];
                                        const count = alertsBySeverity[level]?.length || 0;
                                        return (
                                            <div key={level} className={cn('rounded-xl border p-4', meta.cardTone)}>
                                                <p className="text-xs uppercase tracking-wide text-gray-300">{meta.label}</p>
                                                <p className={cn('mt-2 text-3xl font-bold', meta.countTone)}>{count}</p>
                                            </div>
                                        );
                                    })}
                                </div>
                                ...
                                {topPriorityAlert ? (
                                    <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                                        <p className="text-xs uppercase tracking-wide text-gray-400">Ação prioritária agora</p>
                                        <p className="mt-1 text-sm font-semibold text-white">{topPriorityAlert.title}</p>
                                        <p className="mt-1 text-xs text-gray-300">{topPriorityAlert.description}</p>
                                    </div>
                                ) : null}

                                {(['critical', 'high', 'medium', 'low']).map((level) => {
                                    const list = alertsBySeverity[level] || [];
                                    ...
                                    return (
                                        <div key={`section-${level}`} className="space-y-2">
                                            <div className="flex items-center justify-between">
                                                <h3 className={cn('text-sm font-semibold', meta.sectionTitleTone)}>
                                                    {meta.label}
                                                </h3>
                                                <span className="text-xs text-gray-400">{list.length} alerta(s)</span>
                                            </div>

                                            <div className="space-y-2">
                                                {list.map((alertItem) => (
                                                    <div key={alertItem.id} className={cn('rounded-xl border p-4', meta.cardTone)}>
                                                        <p className="text-sm font-semibold text-white">{alertItem.title}</p>
                                                        <p className="mt-1 text-xs text-gray-300">{alertItem.description}</p>
                                                        {alertItem.details?.length ? (
                                                            <div className="mt-3 rounded-lg border border-white/10 bg-black/20 p-3 space-y-1">
                                                                {alertItem.details.map((detail, idx) => (
                                                                    <p key={`${alertItem.id}-${idx}`} className="text-xs text-gray-200">• {detail}</p>
                                                                ))}
                                                            </div>
                                                        ) : null}
                                                    </div>
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <label className="inline-flex items-center gap-2 text-sm text-gray-300">
                                        <input
                                            type="checkbox"
                                            checked={!!storeCreateForm.allow_monthly}
                                            onChange={(e) => setStoreCreateForm((prev) => ({ ...prev, allow_monthly: e.target.checked }))}
                                        />
                                        Mensal disponível
                                    </label>
                                    <label className="inline-flex items-center gap-2 text-sm text-gray-300">
                                ...
                                <table className="w-full text-left text-sm text-gray-300">
                                    <thead className="bg-white/5 text-xs uppercase text-gray-300">
                                        <tr>
                                            ...
                                            <th className="px-3 py-3 text-center">Mensal disponível</th>
                                            ...
                                            return (
                                                <tr key={`streaming-${platform.id}`} className="border-b border-white/5 hover:bg-white/5 transition-colors align-top">
                                                    ...
                                                    <td className="px-3 py-3 text-center">
                                                        <input
                                                            type="checkbox"
                                                            checked={edit.allow_monthly !== false}
                                                            onChange={(e) => setStoreEdits((prev) => ({
                                                                ...prev,
                                                                [streamingProduct?.id]: { ...prev[streamingProduct?.id], allow_monthly: e.target.checked },
                                                            }))}
                                                            disabled={!streamingProduct}
                                                        />
                                                    </td>
                                                ...
                                                <tr key={platform.id} className="border-b border-white/5 hover:bg-white/5 transition-colors align-top">
                                                    ...
                                                    <td className="px-3 py-3 text-center">
                                                        <input
                                                            ...
                                                            onChange={(e) => setStoreEdits((prev) => ({
                                                                ...prev,
                                                                [product?.id]: { ...prev[product?.id], allow_monthly: e.target.checked },
                                                            }))}
                                                            disabled={!product}
                                                        />
                                                    </td>
            ...
            >
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-400">Carteira de Créditos</label>
                        <div className="rounded-xl border border-white/10 bg-white/5 p-3 space-y-3">
                            ...
                            <div className="flex gap-2">
                                ...
                                <Button
                                    type="button"
                                    onClick={grantWalletCreditToUser}
                                    isLoading={walletCreditSaving}
                                >
                                    Adicionar
                                </Button>
                            </div>
